...
const ExpenseList = ({ expenses, onEdit, onDelete, onAdd }) => {
  ...
  // Pie chart data
  const accountMap = {};
  expenses.forEach(({ account, amount }) => {
    ...
    <div style={{ marginTop: "40px" }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={2}>
        <Typography variant="h5" fontWeight="bold" gutterBottom>
          <AccountBalanceWalletIcon /> EXPENSE HISTORY
        </Typography>
        <Button
          onClick={onAdd}
          sx={{
      ...
      <Paper sx={{ height: 420, width: "100%" }}>
        <DataGrid
          rows={expenses.map((expense) => ({ id: expense._id, ...expense }))}
          columns={columns}
      ...
      <div className="Chart">
        <BarChart
          ...
          series={[{ data: barChartData.map((item) => item.value), label: "Expense by Category" }]}
          height={300}
          width={500}
